...
- [ADR-0015: Tsumego is a game mode, and the menu reports it rather than offering it](0015-tsumego-is-a-game-mode.md) — *Accepted; implemented. Settles the tsumego design ADR-0007 left open, and makes `tsumego + EXPLORE` unrepresentable. Its two deferred UX questions are taken up by ADR-0016*
- [ADR-0016: The Review menu holds no state](0016-the-review-menu-holds-no-state.md) — *Accepted; implemented. Applies ADR-0012 to the prisoner counts, which were in three places at once and invisible on four of six shaders. Carries a correction: the zero is drawn*
- [ADR-0017: A tunable shader parameter is declared in the configuration](0017-tunable-shader-parameters-are-declared-in-configuration.md) — *Accepted; M1 (booleans) implemented. Extends ADR-0011 to the shader parameter editor, and separates a scene's capability from the user's setting so the bowls toggle cannot take the prisoner counts with it. Its log records the prisoners being in the lids, and a two-identity bug committed while implementing the decision against them*
