...
{
	float gamma = 2.3386;
	vec3 cl = pow(render(rool, normalize(rdbl), bgA),vec3(gamma));
	vec3 cr = pow(render(roor, normalize(rdbr), bgA),vec3(gamma));
	glFragColor.rgb = pow(vec3(0.9707,0.0,0.0)*cl.rgb + vec3(0.0,0.7709,0.1989)*cr.rgb,vec3(1.0/gamma));
}
